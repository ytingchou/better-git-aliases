...
- `git remote-capabilities`: show the current provider's supported remote actions and fallback behavior
- `git remote-menu [base]`: open a focused remote actions menu
- `git remote-status [base]`: show the current branch, base branch, provider, upstream state, compare readiness, and the recommended next step
